...
const modal = ({setCheckLoad,typeModal,dataTab,setTab}) => {
  ...
  useEffect(()=>{
    if(resultAlert){
      const loaddata = async() =>{
        switch(alertType){
          case 'create':
            const data = await axios.post(`${process.env.URL_API}/api/vat/create`,dataLoad)
            if(data.data.success){
              setTab(false)
            }
            break
          default:
            const dataUp = await axios.put(`${process.env.URL_API}/api/vat/update/${dataLoad._id}`,dataLoad)
  ...
  const handleUpdateData = async(e)=>{
    switch(e.target.name){
      case 'percent':
        setDataLoad({...dataLoad,percent: e.target.value})
        break
    }
    ...
    <>
      ...
      } 
      {
        ...
        <li className='flex flex-col mb-9'>
          <label className='font-semibold'>Percent*</label>
          <input disabled={disabled} name='percent' className='outline-none bg-slate-200 p-3' value={dataLoad.percent} onChange={handleUpdateData} placeholder='Phần trăm' type='number'/>
        </li>       
      </ul> :''
      }
      {
        typeModal == 'add' ? <button name='create' className='py-3 px-4 bg-yellow-400 rounded-sm float-right font-bold' onClick={(e)=>{
         handleUpdateAlert(e,dataLoad._id)
