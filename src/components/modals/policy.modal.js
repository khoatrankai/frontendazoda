...
const modal = ({setCheckLoad,typeModal,dataTab,setTab}) => {
  ...
  useEffect(()=>{
    ...
    const newloaddata = () => {
      setDataLoad({name:'',url: '',sourceCode:''})
    }
    switch(typeModal){
      case 'add':
        newloaddata()
  ...
  useEffect(()=>{
    if(resultAlert){
      const loaddata = async() =>{
        switch(alertType){
          case 'create':
            const data = await axios.post(`${process.env.URL_API}/api/policy/create`,dataLoad)
            if(data.data.success){
              setTab(false)
            }
            break
          default:
            const dataUp = await axios.put(`${process.env.URL_API}/api/policy/update/${dataLoad._id}`,dataLoad)
  ...
  const handleUpdateCode = (type,data)=> {
    if(type === 'sourceCode'){
      setDataLoad({...dataLoad,sourceCode: data})
    }
  }
  return (
    <>
      <div className='bg-white h-full w-full overflow-y-scroll px-16 py-12 relative'>
      ...
      } 
      {
        ...
        <li className='flex flex-col mb-9'>
          <label className='font-semibold'>Tên chính sách*</label>
          <input disabled={disabled} name='name' className='outline-none bg-slate-200 p-3' value={dataLoad.name} onChange={handleUpdateData} placeholder='Tên chính sách' type='text'/>
        </li>
        ...
        <li className='flex flex-col mb-9'>
          <label className='font-semibold'>URL</label>
          <TextEditor disabled={disabled} setDataChange={handleUpdateCode} name={'sourceCode'} value={dataLoad.sourceCode}/>
        </li> 
      </ul> :''
      }
      {
