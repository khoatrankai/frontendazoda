    ...
    useEffect(()=>{
        ...
        if(checkLoad && funcAll.read && funcAll.pathname === document.location.pathname)
        {
            const loaddata = async()=>{
                const data = await axios.get(`${process.env.URL_API}/api/admin/list`)
    ...
    <>
        ...
        <div className='bg-white p-4 h-5/6 rounded-md'>
            <div className='flex justify-between'>
                <h1 className='font-semibold text-2xl'>Tất cả nhân viên</h1>
                <div className='w-1/3'>
            ...
            <div className='table px-8 pt-4 w-full' >
                    ...
                    <tr className='h-12'>
                                ...
                                <th className='w-1/4'>Avatar</th>
                                <th className='w-1/6'>Tài khoản</th>
                                <th className='w-1/6'>Tên nhân viên</th>
                                <th className='w-1/6'>Trang quản lý</th>
            ...
            <div className='pl-8 pr-4 w-full h-96 overflow-y-scroll' >
                    ...
                    <tbody>
                            {  dataAdmin.length >0 ?
                                dataAdmin.map((dt,index) => {
                                    return(
                                        <tr key={dt._id}  className='hover:bg-black hover:text-white text-center font-medium'>
                                            <td>{index + 1}</td>
                                            <td className='flex justify-center items-center h-full'> <img className='w-full h-20' src={dt.avatar}/> </td>
                                            <td>{dt.username}</td>
                                            <td>{dt.name}</td>
                                            <td>{dt.pageList.length}</td>
                                            <td>
                                                {
        ...
        {
            tabModal?
            <LayoutModal children={<PermissionModal setCheckLoad={setCheckLoad} dataTab={dataTab} setTab={setTabModal} typeModal={typeModal}/>}/> : ''
        }
    </>
  )
}
