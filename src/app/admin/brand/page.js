    ...
    <>
        ...
        <div className='bg-white p-4 h-5/6 rounded-md'>
            <div className='flex justify-between'>
                <h1 className='font-semibold text-2xl'>Tất cả thương hiệu</h1>
                <div className='w-1/3'>
                                ...
                                dataBrand.map((dt,index) => {
                                    return(
                                        <tr key={dt._id}  className='hover:bg-black hover:text-white text-center font-medium'>
                                            <td>{index + 1}</td>
                                            <td>{dt.name}</td>
                                            <td className='flex justify-center items-center h-full'> <img className='w-full h-20' src={`${process.env.URL_API}${dt.logo}`}/> </td>
                                            <td>{dt.code}</td>
                                            <td>{dt.listProduct.length}</td>
                                            <td>{dt.listCategory.length}</td>
        ...
        {
            tabModal?
            <LayoutModal children={<BrandModal setCheckLoad={setCheckLoad} dataTab={dataTab} setTab={setTabModal} typeModal={typeModal}/>}/> : ''
        }
    </>
  )
}
