...
import VatModal from '@/components/modals/vat.modal'
...
const page = () => {
    ...
    useEffect(()=>{
        ...
        if(checkLoad && funcAll.read && funcAll.pathname === document.location.pathname)
        {
            const loaddata = async()=>{
                const data = await axios.get(`${process.env.URL_API}/api/vat/list`)
                if(data.data.success){
                    setDataVat(data.data.data)
                }
            }
            loaddata()
            setCheckLoad(false)
  ...
  return (
    <>
        ...
        <div className='bg-white p-4 h-5/6 rounded-md'>
            ...
            <div className='table px-8 pt-4 w-full' >
                    ...
                    <tr className='h-12'>
                                ...
                                <th className='w-1/4'>Phần trăm VAT</th>
                                <th className='w-1/3'>Số lượng sản phẩm áp dụng</th>
        ...
        {
            tabModal?
            <LayoutModal children={<VatModal setCheckLoad={setCheckLoad} dataTab={dataTab} setTab={setTabModal} typeModal={typeModal}/>}/> : ''
        }
    </>
  )
}
